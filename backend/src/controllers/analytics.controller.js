...
async function getBottleneckAnalysis(req, res, next) {
  try {
    ...
    // Get help area request counts by label
    const skillAreas = await prisma.skillArea.findMany({
      select: { id: true, name: true, label: true },
    });
    const labelByName = new Map(skillAreas.map((sa) => [sa.name, sa.label]));

    // Get help request counts (recent: last 7 days)
    const sevenDaysAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
    const helpRequests = await prisma.dailySelfReport.findMany({
      where: {
        helpNeededAreaId: { not: null },
        createdAt: { gte: sevenDaysAgo },
      },
      include: { helpNeededArea: { select: { label: true } } },
    });

    // Count help requests by label
    const helpRequestsByLabel = new Map();
    for (const req of helpRequests) {
      const label = req.helpNeededArea?.label || 'Unknown';
      helpRequestsByLabel.set(label, (helpRequestsByLabel.get(label) || 0) + 1);
    }
    ...
    const bottlenecks = categories
      .map((category) => {
        ...
        const helpRequestCount = helpRequestsByLabel.get(label) || 0;
