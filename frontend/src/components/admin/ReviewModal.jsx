...
function ReviewForm({ trainee, reviewNumber, existingReview, onSaved, onClose }) {
  ...
  useEffect(() => {
    ...
    if (existingReview) {
      ...
      setReviewDate(existingReview.reviewDate ? existingReview.reviewDate.split('T')[0] : '');
  ...
  async function handleSave() {
    try {
      ...
      const data = {
        scores,
        notes,
        reviewDate: reviewDate ? new Date(reviewDate + 'T00:00:00').toISOString() : new Date().toISOString(),
        decision,
