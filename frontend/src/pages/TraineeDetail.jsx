...
function formatDate(isoString) {
  ...
  // If it's a DATE-only string (YYYY-MM-DD with no time), don't apply timezone conversion
  if (isoString.match(/^\d{4}-\d{2}-\d{2}$/)) {
    const [year, month, day] = isoString.split('-');
    return new Date(year, month - 1, day).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' });
  }
  
  // Otherwise, parse as full ISO datetime
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        ...
        <div className="mt-2 flex items-start justify-between">
          ...
          {user && <IssueSelfReportCredentialsModal traineeId={traineeId} onIssued={loadDetail} />}
        </div>
      </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {recentSelfReports.map((report) => (
                  <tr key={report.id}>
                    ...
                    <td className="px-3 py-2">{report.helpNeededArea?.label || '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {performanceMetrics.map((metric) => {
            ...
            const metricDisplayName = METRIC_DISPLAY_NAMES[metric.category];
            const helpRequested = (recentSelfReports || []).some(
              (report) => report.helpNeededArea?.label === metricDisplayName
            );
            return (
