...
function DailyReportForm() {
  ...
  async function loadSkillAreas() {
    try {
      const areas = await api.getSkillAreas();
      setSkillAreas(areas);
    } catch (err) {
  ...
  async function handleSubmit(event) {
    ...
    if (!formData.snapCasesDone || !formData.medicalCasesDone || !formData.casesCertified || !formData.casesPending) {
      setErrorMessage('All case fields are required');
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        ...(formData.skillAreaId && { helpNeededAreaId: formData.skillAreaId }),
      };

      await api.submitSelfReport(token, payload);
      setFormData({ snapCasesDone: '', medicalCasesDone: '', casesCertified: '', casesPending: '', skillAreaId: '' });
      ...
      <div className="rounded-lg bg-white border border-slate-200 p-6 shadow-sm">
        ...
        <div className="space-y-4">
          ...
          <div className="pt-4 border-t border-slate-200">
            <label className="block text-sm font-medium text-slate-700 mb-1">
              Which area do you need more help with? <span className="text-xs text-slate-500">(Optional)</span>
            </label>
            <select
              value={formData.skillAreaId}
              onChange={(e) => setFormData({ ...formData, skillAreaId: e.target.value })}
              className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
            >
            ...
            <p className="text-xs text-slate-500 mt-1">Your trainer will see this and can provide targeted coaching</p>
          </div>
        </div>
        ...
        {courses.map((completion) => (
          <div key={completion.id} className="rounded-lg border border-slate-200 bg-white p-4">
            ...
            {completion.status !== 'COMPLETED' && (
              <div className="mt-4">
                <button
                  onClick={() => markCourseComplete(completion.courseId)}
                  disabled={savingCourseId === completion.courseId}
                ...
                >
                  {savingCourseId === completion.courseId ? 'Marking Complete...' : 'Mark Course Complete'}
                </button>
              </div>
            )}
              ...
              <div className="mt-4 rounded-md bg-green-50 border border-green-200 p-2">
                ...
                {completion.completedAt && (
                  <p className="text-xs text-green-700">
                    Marked on {formatDate(completion.completedAt)}
                  </p>
                )}
              </div>
            )}
