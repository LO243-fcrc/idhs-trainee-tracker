...
export default function MetricsSettings() {
  ...
  function handleDeleteMetric(metricKey) {
    if (!window.confirm('Delete this metric? This action cannot be undone.')) {
      return;
    }
    setMetrics(metrics.filter(m => m.key !== metricKey));
  }

  function handleCreateMetric(newMetric) {
    setCreateError('');

    // Check for duplicate key
    if (metrics.some(m => m.key === newMetric.key)) {
      setCreateError(`A metric with key "${newMetric.key}" already exists.`);
      return;
    }
    ...
    <AppLayout>
      <div className="space-y-6 p-6">
        ...
        {metrics.length === 0 ? (
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
            <p className="text-sm text-slate-600">
              <strong>{metrics.length} metrics</strong> currently defined. Add, edit, or delete metrics as needed.
            </p>
          </div>
        )}
      </div>
